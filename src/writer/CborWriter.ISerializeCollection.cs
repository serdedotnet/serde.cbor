...
partial class CborWriter<TWriter>
{
    private sealed class SerCollection(CborWriter<TWriter> writer) : ITypeSerializer
    {
        ...
        public void WriteBool(ISerdeInfo typeInfo, int index, bool b) => writer.WriteBool(b);

        public void WriteChar(ISerdeInfo typeInfo, int index, char c) => writer.WriteChar(c);

        public void WriteDecimal(ISerdeInfo typeInfo, int index, decimal d) =>
            writer.WriteDecimal(d);

        public void WriteF32(ISerdeInfo typeInfo, int index, float f) => writer.WriteF32(f);

        public void WriteF64(ISerdeInfo typeInfo, int index, double d) => writer.WriteF64(d);
        ...
        public void WriteNull(ISerdeInfo typeInfo, int index) => writer.WriteNull();

        public void WriteString(ISerdeInfo typeInfo, int index, string s) => writer.WriteString(s);
        ...
        public void WriteValue<T>(ISerdeInfo typeInfo, int index, T value, ISerialize<T> serialize)
            where T : class? => serialize.Serialize(value, writer);
